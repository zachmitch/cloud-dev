...
async function clearArtistWorks(req, artist){
  ...
  for(var j = 0; j < workSz; j++){
    await get_work(req, artist.works[j].id).then((work)=>{
      ...
      edit_work(work.id, work.name, work.year, work.location, "", work.owner);
      console.log("finished with: " + work.name);
            ...
            return get_work(req,req.params.id).then( workExists => {
              ...
              if(workExists){
                ...
                if(workExists.owner !== req.user.name){
                  ...
                  console.log("CLEARING FROM ARTISTS WORK")
            ...
            return get_work(req, req.params.work_id).then( workExists => {
                ...
                if(workExists){ // Here we know both artist/artwork exist
                  ...
                  if(workExists.artist == ""){
                        ...
                        if(workExists.owner !== req.user.name){
                          console.log("ERR: username: " + req.user.name);
                          res.status(403).end();
                        } else {
                          var a = {};
                          a.name = artist.name;
                    ...
                    console.log("Already has an artist");
                    res.status(403).end();
                    return;
                  }
...
router.put('/newuser', function(req, res){
  res.set('Accept', 'POST');
  res.status(405).send("Can only create new user.");
});

//Try to delete
router.delete('/newuser', function(req, res){
  res.set('Accept', 'POST');
  res.status(405).send("Can only create new user.");
});
...
router.put('/login', function(req, res){
  res.set('Accept', 'POST');
  res.status(405).send("Can only POST login.");
});
